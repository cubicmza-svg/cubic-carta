...
const ESTADOS_D = ['pendiente', 'en proceso', 'revision', 'entregado'];
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-8">
      <div className="flex items-center justify-between mb-5">
        <h2 className="font-bebas text-2xl tracking-widest text-gray-800">DISENO</h2>
        <button
      ...
      {showForm && (
        <div className="rounded-2xl p-6 mb-6" style={{ background: '#f5f3ff', border: '2px solid #ddd6fe' }}>
          <h3 className="font-bebas text-xl tracking-widest mb-4" style={{ color: GU_MKT }}>
            {editId ? 'EDITAR PEDIDO' : 'NUEVO PEDIDO DE DISENO'}
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                placeholder="Nombre o Glow Up Deco"
              />
            </div>
            <div>
              <label className="font-dm text-xs font-semibold text-gray-600 uppercase tracking-wider block mb-1">Tipo</label>
              <select
                value={tipo}
            ...
            <div>
              <label className="font-dm text-xs font-semibold text-gray-600 uppercase tracking-wider block mb-1">Formato</label>
              <select
                value={formato}
                ...
                style={{ borderColor: border }}
              >
                {FORMATOS.map(o => <option key={o}>{o}</option>)}
              </select>
            </div>
            <div>
              ...
              <select
                ...
                style={{ borderColor: border }}
              >
                {ESTADOS_D.map(o => <option key={o}>{o}</option>)}
              </select>
            </div>
            <div className="sm:col-span-2">
              <label className="font-dm text-xs font-semibold text-gray-600 uppercase tracking-wider block mb-1">Descripcion / Brief</label>
              <textarea
                value={descripcion}
                ...
                style={{ borderColor: border }}
                placeholder="Que queres mostrar, colores, texto, referencias..."
              />
            </div>
            <div className="sm:col-span-2">
              ...
              <textarea
                ...
                className="w-full px-3 py-2 rounded-xl border font-dm text-sm bg-white outline-none resize-none"
                style={{ borderColor: border }}
                placeholder="Fecha limite, link de referencia..."
              />
            </div>
          </div>
        ...
        {items.map(d => (
          ...
          >
            <div className="flex items-start justify-between gap-2 mb-2">
              ...
              <span
                className="font-dm text-xs uppercase tracking-wider px-2.5 py-1 rounded-full font-semibold shrink-0"
                style={{ background: ESTADO_BG[d.estado] || '#f3f4f6', color: ESTADO_FG[d.estado] || '#6b7280' }}
              >
                {d.estado}
              </span>
            </div>
            <p className="font-dm text-xs text-gray-600 mb-3 leading-relaxed line-clamp-3">{d.descripcion}</p>
            {d.notas && <p className="font-dm text-xs text-gray-400 mb-3 italic">{d.notas}</p>}
            <div className="flex flex-wrap gap-1.5 mb-3">
              {ESTADOS_D.filter(e => e !== d.estado).map(e => (
                <button
                  key={e}
                  onClick={() => cambiarEstado(d, e)}
                  className="font-dm text-xs uppercase tracking-wider px-2 py-0.5 rounded-full font-semibold"
                  style={{ background: ESTADO_BG[e], color: ESTADO_FG[e] }}
                >
                  {e}
                </button>
              ))}
            ...
            <div className="flex gap-2 justify-end">
              <button onClick={() => startEdit(d)} className="font-dm text-xs text-gray-400 hover:text-gray-700">editar</button>
              <button onClick={() => del(d.id)} className="font-dm text-xs text-red-300 hover:text-red-500">eliminar</button>
            </div>
          </div>
        ))}
        {!loading && items.length === 0 && (
          <p className="font-dm text-sm text-gray-400 py-10 text-center sm:col-span-2">
            Sin pedidos de diseno aun
          </p>
        )}
      </div>
    </div>
